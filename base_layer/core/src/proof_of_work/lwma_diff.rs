...
mod test {
    ...
    async fn response_test(mut lwma: LinearWeightedMovingAverage, hash_change: f32, difficulty: Difficulty)-> usize{
        let mut time = 0;
        ...
        let mut blocks = 1;
        while (new_diff.as_u64() as f32) / (stop_diff.as_u64() as f32) < 0.99 ||  (new_diff.as_u64() as f32) / (stop_diff.as_u64() as f32) > 1.01 {
            blocks += 1;
        ...
        output.write_all(&buff).await.unwrap();
        blocks
    }

    /// Sample a block solve time from an exponential distribution with the given `mean`, using
    /// inverse transform sampling. Real proof-of-work block times are the inter-arrival times of a
    /// Poisson process, which are exponentially distributed — not uniform between a min and a max.
    /// For `U ~ Uniform(0, 1]`, `-mean * ln(U)` is exponentially distributed with mean `mean`.
    fn exp_solve_time(rng: &mut impl rand::RngExt, mean: f64) -> i64 {
        // `1.0 - random()` lands in (0, 1], which excludes 0 so `ln` is always finite.
        let u = 1.0 - rng.random::<f64>();
        (-mean * u.ln()) as i64
    }

    async fn variance_test(mut lwma: LinearWeightedMovingAverage, difficulty: Difficulty, amount: usize)-> (u64, u64, u64, u64){
        let mut output = tokio::fs::OpenOptions::new()
            .create(true)
            .append(true)
            .open("variance.csv")
            .await
            .unwrap();
        let block_window = lwma.block_window();
        let mut time:i64 = 0;
        while !lwma.is_full(){
            time += 100;
        ...
        // The first block is mined at the target difficulty, so its mean solve time is the target
        // time (100). Draw the actual solve time from an exponential distribution around that mean.
        let mut new_time = exp_solve_time(&mut rng, 100.0);

        time += new_time;
        lwma.add(EpochTime::from(time as u64), difficulty).unwrap();
        let mut new_diff = lwma.get_difficulty().unwrap();
        loop{
            // Mean solve time = target_time * (chosen difficulty / true difficulty). Real block
            // times are exponentially distributed around this mean, not uniform noise around it.
            let mean = (new_diff.as_u64() as f64 / difficulty.as_u64() as f64) * 100.0;
            new_time = exp_solve_time(&mut rng, mean);
            time += new_time;
        ...
        // Prefix each row with its block window so the CSV is self-describing.
        // Use `as_u64()` so the raw integer is written; the `Display` impl inserts thousands
        // separators which would corrupt the CSV.
        let mut data = format!("{},", block_window);
        for diff in &difficulties {
            data.push_str(&format!("{},", diff.as_u64()));
        }
        let mut buff = Vec::new();
        writeln!(
            buff, "{}", data);
    ...
    #[tokio::test]
    async fn respnse_lwma(){
        // Start with a clean file; response_test appends one row per call.
        File::create("response.csv").await.unwrap();
        ...
        for hash in hash_rates {
            ...
            for window in block_windows {
                ...
                let blocks = response_test(lwma, hash, Difficulty::from_u64(100_000).unwrap()).await;
    ...
    #[tokio::test]
    async fn variance_lwma(){
        File::create("variance.csv").await.unwrap();
        let block_windows = [30, 45, 60, 80, 90, 110, 150];
        for window in block_windows {
            println!("block window: {}", window);
            let lwma = LinearWeightedMovingAverage::new(window, 100).unwrap();
            let (mean, avg, min, max) = variance_test(lwma, Difficulty::from_u64(100_000_000).unwrap(), 10000).await;
